    ...
    Raphael.fn.freeTransform = function(subject, options, callback) {
        ...
        var ft = subject.freeTransform = {
            ...
            attrs: {
                ...
                translate: { x: 0, y: 0 },
                ratio: 1,
                draggLockedOnLeft: false,
                draggLockedOnRight: false,
                draggLockedOnTop:false,
                draggLockedOnBottom:false
            },
            axes: null,
            bbox: null,
            callback: null,
        ...
        ft.updateHandles = function() {
            ...
            ft.axes.map(function(axis) {
                if ( ft.handles[axis] ) {
                    ...
                    if ( ft.opts.boundary ) {
                        var noBoundaryCX = cx, noBoundaryCY = cy;
                        cx = Math.max(Math.min(noBoundaryCX, ft.opts.boundary.x + ( ft.opts.boundary.width  || getPaperSize().x )), ft.opts.boundary.x);
                        cy = Math.max(Math.min(noBoundaryCY, ft.opts.boundary.y + ( ft.opts.boundary.height || getPaperSize().y )), ft.opts.boundary.y);
                    }
            ...
            if ( ft.opts.draw.indexOf('bbox') >= 0 && ( ft.opts.scale.indexOf('bboxCorners') !== -1 || ft.opts.scale.indexOf('bboxSides') !== -1 ) ) {
                ft.handles.bbox.map(function(handle) {
                    handle.element.drag(function(dx, dy) {
                        ...
                        // Check boundaries
                        if ( !isWithinBoundaries().x || !isWithinBoundaries().y ) { ft.attrs = previous;/* ft.attrs.isDraggLockedByBoundaries=true */}
        ...
        function applyLimits(bbox) {
            ...
            if ( ft.opts.boundary ) {
                ...
                if ( ft.attrs.center.x + ft.attrs.translate.x < b.x            ) {
                    ...
                    ft.attrs.draggLockedOnLeft=true;
                } else {
                    ft.attrs.draggLockedOnLeft=false;
                }
                if ( ft.attrs.center.y + ft.attrs.translate.y < b.y            ) {
                    ft.attrs.translate.y += b.y -            ( ft.attrs.center.y + ft.attrs.translate.y );
                    ft.attrs.draggLockedOnTop=true;
                } else {
                    ft.attrs.draggLockedOnTop=false;
                }
                if ( ft.attrs.center.x + ft.attrs.translate.x > b.x + b.width  ) {
                    ...
                    ft.attrs.draggLockedOnRight=true;
                } else {
                    ft.attrs.draggLockedOnRight=false;
                }
                if ( ft.attrs.center.y + ft.attrs.translate.y > b.y + b.height ) {
                    ...
                    ft.attrs.draggLockedOnBottom=true;
                } else {
                    ft.attrs.draggLockedOnBottom=false;
                }
            }
