...
@Provides
@Instantiate
public class DirectoryPluginFacesMBeanRegistryImpl implements DirectoryPluginFacesMBeanRegistry {
    ...
    public void addPluginFacesMBeanConfigsToServletContext() {
        if (servletContext!=null) {
            for (URL url : pluginFacesConfigToAdd) {
                registerFromDocument(parseXML(url));
    ...
    public void delPluginFacesMBeanConfigsFromServletContext() {
        if (servletContext != null) {
            for (URL url : pluginFacesConfigToDel) {
                try {
                    unregisterFromDocument(parseXML(url));
    ...
    private void registerFromDocument(Document dom) {
    ...
    private void unregisterFromDocument(Document dom) throws IllegalAccessException, InstantiationException, ClassNotFoundException {
        String namespace = dom.getDocumentElement().getNamespaceURI();
    ...
    private void delManagedBean(BeanManager beanManager, Node managedBean) throws ClassNotFoundException, IllegalAccessException, InstantiationException {
    ...
    private Map<String, String> getTextMap(List<Node> list) {
